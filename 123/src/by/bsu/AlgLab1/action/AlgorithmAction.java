...
public class AlgorithmAction {
    ...
    {
        for (int i = begin; i <= end; ++i) { // 1 + 3*N
            int key = arr[i];                // 2*N
            int j = i - 1;                   // 2*N
                        // цикл в худшем случае пройдет N(N-1) / 2
            while (j >= 0 && arr[j] > key) { // 4
                arr[j + 1] = arr[j];         // 4
                j = j - 1;                   // 2
            }
                        // итого цикл 10N(N-1) / 2
            arr[j + 1] = key;                  //N*3
        }
        // в итоге 1+3N+2N+2N+10N(N-1)/2+3N = 1+10N+10N(N-1)/2 ~ N^2
    }
    ...
    public int findTheOptimalK(int[][] arrays, Sort sortObj,int k, boolean toPrint) {
        ...
        if(toPrint) System.out.println("Minimum execution time: " + minTimeInMillis + " millis with K = " + bestK);
        return (int) bestK;
    }
